...
#[macro_export]
macro_rules! cat {
    ($amt: expr, $x:expr, $y:expr) => {
        ($x.wrapping_shl($amt)) | $y
    };
}
...
macro_rules! join {
    ($ty: ty, $n:expr, $xs:expr) => {
        $xs.fold(0 as $ty, |b, x| {
            $crate::cat!($n, b, <$ty>::from((&x).as_arg()))
        })
    };
}
    ...
    ($ty:ty, $sign_ty:ty, $from_uint:ident, $w:tt) => {
        ...
        impl StreamBits for $ty {
            ...
            fn from_bits(bits: &mut impl Stream<bool>) -> Self {
                bits.take(Self::BITS as usize)
                    .fold(0, |x, b| if b { (x << 1) | 1 } else { x << 1 })
            }
        }
        ...
        impl SignedCmp for $ty {
            fn signed_lt(x: $ty, y: $ty) -> bool {
                (x as $sign_ty) < (y as $sign_ty)
            }
        }
        ...
        impl Ring for $ty {
            ...
            fn from_integer(_n: Self::Length, x: &num::BigInt) -> Self {
                ...
                let res: $ty = if w <= 32 {
                    ...
                    it.take(w / 32)
                        .rev()
                        .fold(0, |x, d| x.wrapping_shl(32) | (d as Self))
                };
                match x.sign() {
        ...
        impl Integral for $ty {
            ...
            fn to_integer(x: $ty) -> num::BigInt {
                x.into()
            }
            fn div(x: $ty, y: $ty) -> $ty {
                x / y
            }
            fn modulo(x: $ty, y: $ty) -> Self {
                x % y
            }
        }
        ...
        impl SeqOwned for $ty {
            ...
            fn seq_update(self, j: usize, v: bool) -> Self {
                ...
                assert!(
                    j < w,
                    "seq_udpate: Index {} is too large for `{}`",
                    j,
                    stringify!($ty)
                );
                if v {
...
pub mod test {
    ...
    fn pow_ok() {
        ...
        for _i in 0..1000000 {
            ...
            // only large ones.
            let base = rng.r#gen::<u8>() >> rng.r#gen_range(0..8);
            let exp = rng.r#gen::<usize>() >> rng.r#gen_range(0..64);
            assert_eq!(
                u8::exp_usize(base, exp),
                wrapping_pow_u64_ref(base, exp),
                "base = {base}, exp = {exp}"
            );
        }
    }
