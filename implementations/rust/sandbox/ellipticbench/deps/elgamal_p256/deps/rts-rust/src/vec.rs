...
impl<T: Type> Sequence for &[T] {
    ...
    where
        T: Zero,
    {
        Vec::<T>::from_fn(self.seq_length(), |i| {
            if i < amt {
                T::zero(n.clone())
            } else {
                self.seq_index(i - amt)
            }
        })
    }
    ...
    fn seq_shift_right_signed(self, amt: usize) -> Self::Owned {
        Vec::<T>::from_fn(self.seq_length(), |i| {
            self.seq_index(if i < amt { 0 } else { i - amt })
        })
    }
...
impl<T: Logic> Logic for Vec<T> {
    ...
    fn xor(x: Self::Arg<'_>, y: Self::Arg<'_>) -> Self {
        x.iter()
            .zip(y.iter())
            .map(|(a, b)| T::xor(a.as_arg(), b.as_arg()))
            .collect()
    }
    fn and(x: Self::Arg<'_>, y: Self::Arg<'_>) -> Self {
        x.iter()
            .zip(y.iter())
            .map(|(a, b)| T::and(a.as_arg(), b.as_arg()))
            .collect()
    }
    fn or(x: Self::Arg<'_>, y: Self::Arg<'_>) -> Self {
        x.iter()
            .zip(y.iter())
            .map(|(a, b)| T::or(a.as_arg(), b.as_arg()))
            .collect()
    }
}
...
impl<T: Ring> Ring for Vec<T> {
    ...
    fn mul(x: Self::Arg<'_>, y: Self::Arg<'_>) -> Self {
        x.iter()
            .zip(y.iter())
            .map(|(a, b)| T::mul(a.as_arg(), b.as_arg()))
            .collect()
    }
    ...
    fn sub(x: Self::Arg<'_>, y: Self::Arg<'_>) -> Self {
        x.iter()
            .zip(y.iter())
            .map(|(a, b)| T::sub(a.as_arg(), b.as_arg()))
            .collect()
    }
    ...
    fn add(x: Self::Arg<'_>, y: Self::Arg<'_>) -> Self {
        x.iter()
            .zip(y.iter())
            .map(|(a, b)| T::add(a.as_arg(), b.as_arg()))
            .collect()
    }
...
impl<T: Eq> Eq for Vec<T> {
    fn eq(x: Self::Arg<'_>, y: Self::Arg<'_>) -> bool {
        ...
        x.iter()
            .zip(y.iter())
            .all(|(x, y)| T::eq(x.as_arg(), y.as_arg()))
    }
}
...
#[cfg(feature = "proptest_strategies")]
pub fn any_vec<T: fmt::Debug>(
    any_elem: impl Strategy<Value = T>,
    size: usize,
