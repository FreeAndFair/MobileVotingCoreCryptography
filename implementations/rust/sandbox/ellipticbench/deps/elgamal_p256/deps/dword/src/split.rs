use crate::index::{FromLSB, FromMSB, IndexDir, IndexFrom};
...
impl DWord {
    ...
    pub fn into_iter_words<INDEX: IndexDir>(self, each: usize) -> TraverseWordsOwned<INDEX> {
        TraverseWordsOwned {
            dir: PhantomData,
            vec: self,
            index: 0,
...
impl<'a> DWordRef<'a> {
    ...
    pub fn take(self, amt: usize) -> DWord {
        self.sub_word_msb(amt, 0)
    }
    ...
    /// Generates only full words, so if the sub size does not divide the word
    /// exactly, the left over bits at the end will be ignored.
    pub fn iter_words<INDEX: IndexDir>(self, each: usize) -> TraverseWordsBorrowed<'a, INDEX> {
    ...
    pub fn sub_word<INDEX: IndexDir>(self, sub_bits: usize, index: usize) -> DWord {
...
mod test {
    ...
    fn sub_word<INDEX: IndexDir>() {
        do_test(
            word_and2::<usize, usize>,
            |(x, i0, w0): (DWord, usize, usize)| {
                let i = i0 % (x.bits() + 1);
                let have = x.bits() - i;
                let w = w0 % (have + 1);
                let (xr, a) = x.sem();
