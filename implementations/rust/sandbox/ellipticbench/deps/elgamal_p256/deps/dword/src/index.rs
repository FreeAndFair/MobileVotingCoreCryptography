...
impl IndexDir for FromLSB {
    const DIR: IndexFrom = IndexFrom::Lsb;
}
impl IndexDir for FromMSB {
    const DIR: IndexFrom = IndexFrom::Msb;
}
...
impl<'a> DWordRef<'a> {
    ...
    pub fn index_msb(self, i: usize) -> bool {
        self.index::<FromMSB>(i)
    }
    pub fn index_lsb(self, i: usize) -> bool {
        self.index::<FromLSB>(i)
    }
...
mod test {
    ...
    fn test_leading_zeros() {
        do_test(unary, |x: DWord| {
            ...
            assert!(n <= x.bits());
            for i in 0..n {
                assert_eq!(r.index_msb(i), false)
            }
            if n < r.bits() {
                assert_eq!(r.index_msb(n), true)
            }
            return Some(true);
        })
    }
