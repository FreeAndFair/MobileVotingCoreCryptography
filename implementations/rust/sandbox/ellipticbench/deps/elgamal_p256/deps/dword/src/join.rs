...
impl DWordRef<'_> {
    pub fn append(self, lower_bv: DWordRef<'_>) -> DWord {
        if self.bits() == 0 {
            return lower_bv.clone_word();
        }
        if lower_bv.bits() == 0 {
            return self.clone_word();
        }
        ...
        if pad_upper == 0 {
            out[0..lower_bv.limbs()].copy_from_slice(lower);
        } else {
...
impl DWord {
    pub fn join(parts: usize, each: usize, xs: impl Iterator<Item = DWord>) -> DWord {
